...
/**
 * EventBW schlanker Voll-Listenimport
 *
 * Quellen:
 * - /kategorie/maerkte/
 * - /kategorie/feste/
 ...
 * - page
 *
 * Keine Detailseiten, keine Geodaten, keine km-Berechnung.
 */
...
const USER_AGENT = 'Mozilla/5.0 EventBW-SlimListImporter/4.0 (+https://github.com/Ulli-Buehler/Event-Finder)';
...
function parseDateAndTime(text) {
  ...
  const matches = [...clean.matchAll(/(\d{2})\.(\d{2})\.(\d{4})/g)];
  if (!matches.length) return { startDate: '', endDate: '', time: '' };
  ...
  if (matches.length > 1) {
    ...
    const between = clean.slice(first.index + first[0].length, second.index);
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      time = cleanText(match[1]).replace(/\s*–\s*/g, ' - ').replace(/\s*-\s*/g, ' - ').replace(/\s+Uhr/i, ' Uhr');
      break;
    }
  }
...
function categoryPageUrl(categoryConfig, page) {
  return new URL(page === 1 ? categoryConfig.path : `${categoryConfig.path}page/${page}/`, BASE_URL).toString();
}

function extractReportedCount(html) {
  const text = cleanText(html);
  const match = text.match(/(\d+)\s+Veranstaltungen gefunden/i) || text.match(/(\d+)\s+aktuelle/i);
  return match ? Number(match[1]) : null;
}

function extractCards(html) {
  const beforePagination = html.split(/<h2[^>]*>\s*Posts pagination\s*<\/h2>/i)[0] || html;
  const regex = /<h3[^>]*>\s*<a\s+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>\s*<\/h3>/gi;
  const matches = [...beforePagination.matchAll(regex)];
  return matches.map((match, index) => {
    const start = match.index;
    const end = index + 1 < matches.length ? matches[index + 1].index : beforePagination.length;
    return { href: match[1], titleHtml: match[2], blockHtml: beforePagination.slice(start, end) };
  });
}
...
function extractCity(text, categoryLabel, startDate) {
  ...
  return generic ? cleanText(generic[1]) : '';
}
...
async function collectCategory(categoryConfig) {
  ...
  for (let page = 1; page <= MAX_PAGES_PER_CATEGORY; page++) {
    ...
    try {
      ...
      pages.push({ category: categoryConfig.category, page, url: sourceUrl, ok: true, reportedCount, parsedCount: pageEvents.length });
      if (!pageEvents.length) break;
      events.push(...pageEvents);
      if (reportedCount !== null && events.length >= reportedCount) break;
    } catch (error) {
      pages.push({ category: categoryConfig.category, page, url: sourceUrl, ok: false, error: String(error.message || error) });
      break;
    }
  }
  return { pages, events };
...
function eventsToText(title, events, meta) {
  const lines = [title, `Zieldatum: ${meta.targetDate}`, `Treffer: ${events.length}`, ''];
...
function summaryToText(meta, pages) {
  return [
    'EventBW schlanker Voll-Listenimport', '',
    'Quellen:', '- /kategorie/maerkte/', '- /kategorie/feste/', '',
    'Importierte Event-Felder:', '- title', '- category', '- city', '- startDate', '- endDate', '- time', '- detailUrl', '- sourceUrl', '- page', '',
    'Nicht geladen:', '- Detailseiten', '- Geodaten', '- km-Entfernung', '- Bilder', '- Browser/Playwright', '',
    `Zieldatum: ${meta.targetDate}`, '',
    'Counts:', JSON.stringify(meta.counts, null, 2), '',
...
async function main() {
  ...
  const meta = {
    ...
    counts: {
      ...
      finalMaerkte: targetDateEvents.filter(event => event.category === 'maerkte').length,
      finalFeste: targetDateEvents.filter(event => event.category === 'feste').length,
      rawWithTime: rawEvents.filter(event => event.time).length,
      rawWithoutTime: rawEvents.filter(event => !event.time).length,
      finalWithTime: targetDateEvents.filter(event => event.time).length,
      finalWithoutTime: targetDateEvents.filter(event => !event.time).length,
    },
  };

  const debug = { meta, pages: allPages, rawEvents, targetDateEvents };
  ...
  console.log('EventBW slim full list import done.');
