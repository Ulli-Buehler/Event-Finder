...
const TARGET_CENTER = { name: 'Dettingen unter Teck', lat: 48.6158, lon: 9.4535 };
...
const USER_AGENT = 'Mozilla/5.0 EventBW-FastImporter/1.0 (+https://github.com/)';
...
// Static fast geo filter. Extend freely; post-filtering remains possible via debug-output.json.
const CITY_COORDS = {
  ...
  'leonberg': [48.8007, 9.0166],
  'sindelfingen': [48.7075, 9.0035],
  'böblingen': [48.6850, 9.0091],
  'boeblingen': [48.6850, 9.0091],
  'herrenberg': [48.5950, 8.8665],
...
function stripTags(html) {
  return cleanText(String(html || '').replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' '));
}
...
function cityDistanceKm(city) {
  ...
  return Number(haversineKm(TARGET_CENTER, { lat: coord[0], lon: coord[1] }).toFixed(1));
}
...
function parseDateRange(value) {
  ...
  return { raw: text, startDate: start, endDate: end, time: timeMatch ? cleanText(timeMatch[1]) : '' };
}
...
function absoluteUrl(href) {
  if (!href) return '';
  try { return new URL(decodeHtml(href), BASE_URL).toString(); } catch { return ''; }
}
...
function extractListItems(html, category) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const placeMatch = stripTags(block).match(new RegExp(`${category.label}\\s*\\|\\s*([^\n|]+?)\\s+(\\d{2}\\.\\d{2}\\.\\d{4}[^\n]*)`, 'i'))
      || stripTags(block).match(/(?:Feste|Märkte)\s*\|\s*([^\n|]+?)\s+(\d{2}\.\d{2}\.\d{4}[^\n]*)/i);
    ...
    const description = cleanText(text.replace(title, '').replace(category.label, '').replace(city, '').replace(dateRaw, '').replace('Details', ''));

    if (!title || !url || !range.startDate) continue;
    items.push({
      source: 'eventbw', category: category.label, categoryKey: category.key,
      title, city, dateRaw: range.raw, startDate: range.startDate, endDate: range.endDate, time: range.time,
      description, url, listOnly: true,
    });
  }
  return items;
}
...
async function collectCategory(category, targetDate) {
  ...
  for (let page = 1; page <= MAX_PAGES_PER_CATEGORY; page++) {
    ...
    try {
      ...
      debugPages.push({ category: category.key, page, url, ok: false, error: String(err.message || err) });
      break;
    }
    const items = extractListItems(html, category);
    debugPages.push({ category: category.key, page, url, ok: true, items: items.length });
    if (!items.length) break;
    all.push(...items);

    const firstFuture = items.find(x => compareIso(x.startDate, targetDate) > 0 && compareIso(x.endDate, targetDate) > 0);
    const last = items[items.length - 1];
    if (firstFuture && last && compareIso(last.startDate, targetDate) > 0) break;
  }
  return { items: all, debugPages };
}

function isTargetDateEvent(event, targetDate) {
  return event.startDate && event.endDate && compareIso(event.startDate, targetDate) <= 0 && compareIso(event.endDate, targetDate) >= 0;
}

function isMarketOrFestival(event) {
  const hay = normalizeCity(`${event.category} ${event.title} ${event.description}`);
  if (/markt|maerkte|feste|fest|hocketse|hock|dorffest|stadtfest|strassenfest|straßenfest|weinfest|sommerfest|fruehlingsfest|frühlingsfest|kunsthandwerkermarkt|kraemermarkt|krämermarkt|flo(h)?markt/.test(hay)) return true;
  return ['feste', 'maerkte'].includes(event.categoryKey);
}

function isRegionCandidate(event) {
  const dist = cityDistanceKm(event.city);
  if (dist === null) return true; // keep for detail fetch and post-filtering
...
function extractDetail(html, base) {
  ...
  const description = descMatch ? stripTags(descMatch[1]).replace(/^.*?\|.*?·\s*\d{2}\.\d{2}\.\d{4}\s*/i, '').trim() : base.description;
...
async function enrichDetails(items) {
  return mapLimit(items, DETAIL_CONCURRENCY, async item => {
    try {
      ...
      return { ...item, detailError: String(err.message || err) };
    }
  });
}
...
function dedupe(items) {
  ...
  for (const item of items) {
    const key = normalizeCity(`${item.title}|${item.city}|${item.startDate}|${item.url}`);
...
function toText(events, meta) {
  ...
  for (const [i, e] of events.entries()) {
    ...
    lines.push(`   ${e.category} | ${e.city}${e.distanceKm !== null ? ` | ${e.distanceKm} km` : ' | Distanz unbekannt'}`);
    lines.push(`   ${e.startDate}${e.endDate && e.endDate !== e.startDate ? ` - ${e.endDate}` : ''}${e.time ? ` | ${e.time}` : ''}`);
    if (e.venue) lines.push(`   Ort: ${e.venue}`);
...
async function main() {
  ...
  const typeFiltered = dateFiltered.filter(isMarketOrFestival);
  const regionCandidates = typeFiltered.filter(isRegionCandidate);
  ...
  const debug = {
    ...
    notes: [
      ...
      'List pages are parsed first; detail pages are fetched only for target-date regional candidates.',
      'Cities without static coordinates are kept with withinRadius=null for post-filtering.',
      'Extend CITY_COORDS for stricter radius filtering.',
    ],
  };

  await fs.writeFile(path.join(OUT_DIR, 'debug-output.json'), JSON.stringify(debug, null, 2), 'utf8');
  await fs.writeFile(path.join(OUT_DIR, 'debug-output.txt'), toText(finalEvents, meta) + '\n\nDEBUG COUNTS\n' + JSON.stringify(meta.counts, null, 2) + '\n', 'utf8');
